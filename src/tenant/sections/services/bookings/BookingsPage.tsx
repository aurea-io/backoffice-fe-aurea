...
export default function BookingsPage() {
  ...
  const change = async (booking: Booking, status: Booking['status']) => {
    await bookingsApi.updateBooking(booking.id, status);
    load();
  };
      ...
      <Card variant="glass">
        ...
        <CardContent>
          {loading ? (
            ...
            <p className="py-8 text-center text-sm text-zinc-500">No hay reservas para mostrar.</p>
          ) : (
                ...
                <div key={booking.id} className="flex flex-wrap items-center justify-between gap-3 py-4">
                  <div>
                    ...
                    <p className="text-xs text-zinc-500">
                      {booking.date.slice(0, 10)} · {booking.startTime} · {booking.catalogItem?.title ?? 'Servicio'}
                    </p>
                  </div>
