  ...
  return (
    <div className="space-y-6 animate-in fade-in-50 duration-200">
      ...
      <Dialog open={catalogDialog !== null} onOpenChange={(open) => !open && closeCatalogDialog()}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{catalogDialog === 'category' ? 'Nueva categoría' : catalogDialog === 'modifier' ? 'Nuevo grupo de modificadores' : 'Eliminar ítem'}</DialogTitle>
            <DialogDescription>
          ...
          <DialogFooter>
            <Button type="button" variant="outline" size="sm" onClick={closeCatalogDialog}>Cancelar</Button>
            <Button type="button" variant={catalogDialog === 'delete' ? 'danger' : 'primary'} size="sm" onClick={() => void submitCatalogDialog()}>{catalogDialog === 'delete' ? 'Eliminar' : 'Guardar'}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
