...
export default function RestaurantPage() {
  ...
  const load = () => {
    setLoading(true);
    Promise.all([tenantService.getTables(), tenantService.getOrders()])
  ...
  const cycle = async (table: RestaurantTable) => {
    ...
    await tenantService.updateTable(table.id, next);
    load();
  };

  const openQr = async (table: RestaurantTable) => {
    const qr = await tenantService.getTableQr(table.id);
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-zinc-500">
          Estado de mesas y pedidos activos en tiempo real.
        </p>
      </div>

      {loading ? (
        <LoadingSpinner size="lg" label="Cargando salón..." />
      ) : (
