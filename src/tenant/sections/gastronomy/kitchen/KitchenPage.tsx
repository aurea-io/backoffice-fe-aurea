...
export default function KitchenPage() {
  ...
  const load = async () => {
    ...
    setLoading(true);
    try {
      setOrders(await kitchenApi.getKitchenOrders());
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-zinc-500">Pedidos activos ordenados por antigüedad. Se actualiza automáticamente.</p>
      </div>
      {loading ? (
            ...
            <Card key={order.id} variant="glass">
              ...
              <CardContent>
                <p className="mb-4 text-xs text-zinc-500">
                  {order.customerName || 'Cliente'} · {new Date(order.createdAt || Date.now()).toLocaleTimeString()}
                </p>
